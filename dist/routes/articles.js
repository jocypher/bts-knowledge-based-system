...
routes.post("/add", (req, res) => {
    ...
    if (!title || !category_name || !content || !author_name)
    ...
    try {
        ...
        const insertAuthor = db.prepare(`INSERT INTO users(author_name) VALUES(?)`);
        // insert the article
        const insertArticle = db.prepare(`INSERT INTO articles(title, category_id, content, author_id,verification) VALUES(?,?,?,?,?)`);
        // a transaction to insert the article;implement an all or nothing case
        const insertArticleTransaction = db.transaction((title, category_name, content, author_name, verification) => {
...
routes.get("/", (req, res) => {
    ...
    if (getAllArticles.length == 0)
        return res.status(404).json({ message: "No articles present" });
...
routes.get("/:id", (req, res) => {
    ...
    // article not found
    if (!getArticle)
        return res.status(404).json({ message: `Article with id: ${id} does not exist` });
...
routes.delete("/:id/:author_name", (req, res) => {
    const id = Number(req.params.id);
    const author_name = req.params.author_name;
    if (!id)
        return res.status(400).json({ message: `Invalid article id:${id}` });
    if (!author_name)
        return res.status(404).json({ message: `Name of author is required` });
    try {
